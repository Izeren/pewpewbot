...
async def test_with_caption_no_tip(mocker: MockerFixture):
    ...
    # then
    full_view_mock.assert_called_once_with(manager_mock.state.koline.sectors[0], manager_mock.state, 'mocked_caption\n')
    message_mock.reply.assert_called_once_with('mocked_view', parse_mode='Markdown')
...
async def test_with_caption_with_tip(mocker: MockerFixture):
    ...
    full_view_mock = mocker.patch('pewpewbot.views.not_taken_with_tips_for_sector_list', return_value='mocked_view')
    ...
    # then
    full_view_mock.assert_called_once_with(manager_mock.state, 'mocked_caption\n')
    message_mock.reply.assert_called_once_with('mocked_view', parse_mode='Markdown')
...
async def test_no_caption_no_tip(mocker: MockerFixture):
    ...
    # then
    full_view_mock.assert_called_once_with(manager_mock.state.koline.sectors[0], manager_mock.state, '')
    message_mock.reply.assert_called_once_with('mocked_view', parse_mode='Markdown')
...
async def test_no_caption_with_tip(mocker: MockerFixture):
    ...
    # then
    full_view_mock.assert_called_once_with(manager_mock.state, '')
    message_mock.reply.assert_called_once_with('mocked_view', parse_mode='Markdown')
